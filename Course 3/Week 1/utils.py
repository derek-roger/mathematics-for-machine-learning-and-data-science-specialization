...
class your_bday:
    def __init__(self) -> None:
        ...
        self.ax_hist = ax2
        self.dates = [
            (date(2015, 1, 1) + timedelta(days=n)).strftime("%m-%d")
            for n in range(365)
        ]
        self.match = False
        self.bday_str = None
...
def plot_simulated_probs(sim_probs, class_size):
    ...
    sns.scatterplot(
        x=class_size, y=sim_probs, ax=ax, label="simulated probabilities"
    )
...
class third_bday_problem:
    def __init__(self) -> None:
        fig, axes = plt.subplot_mosaic(
            [["top row", "top row"], ["bottom left", "bottom right"]],
            figsize=(10, 8),
        )
        self.fig = fig
    ...
    def add_students(self):
        ...
        while True:
            if self.match:
                ...
                month_str = self.month_names[
                    int(self.match_str.split("-")[0]) - 1
                ]
...
class monty_hall_game:
    def __init__(self) -> None:
        ...
        self.cpoint = self.fig.canvas.mpl_connect(
            "button_press_event", self.click_plot
        )
    ...
    def start(self) -> None:
        ...
        self.prizes = list(
            map(lambda x: "GOAT" if x == 0 else "CAR", list(self.doors))
        )
    ...
    def click_plot(self, event):
        ...
        if event.inaxes in [self.ax]:
            ...
            if (
                (self.choice is not None) or (self.final_choice is not None)
            ) and (not self.ilegal_move):
...
def success_rate_plot(f):
    def _plot(switch, n_iterations):
        ...
        ax.set_title(
            f"Win rate if you {msg} switch doors ({n_iterations} simulations)"
        )
        plt.show()
